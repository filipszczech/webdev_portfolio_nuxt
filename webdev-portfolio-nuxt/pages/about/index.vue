<template>
    <div>
        <section class="md:text-lg mb-16 md:mb-24 text-gray-700 dark:text-gray-400">
            <h1 class="text-3xl md:text-4xl font-bold mb-6 xl:mb-9 text-gray-700 dark:text-gray-400">Informacje o projekcie</h1>
            <p class="md:pr-48">Strona została w pełni zaprojektowana i zaimplementowana przeze mnie. Tworząc projekty dla siebie staram się zadbać o nowoczesny stack technologiczny, jak najlepszy user experience i niskie koszty utrzymania.</p>
            <div class="mt-6 xl:mt-9">
                <h2 class="mb-3">Stack technologiczny strony:</h2>
                <div class="flex gap-4 flex-wrap">
                    <a href="https://vuejs.org/" target="_blank" class="hover:scale-110 transition-all duration-300">
                        <img alt="techstack icon" class="w-9 h-9 lg:w-16 lg:h-16" src="https://icon.icepanel.io/Technology/svg/Vue.js.svg"/>
                    </a>
                    <a href="https://nuxt.com/" target="_blank" class="hover:scale-110 transition-all duration-300">
                        <img alt="techstack icon" class="w-9 h-9 lg:w-16 lg:h-16" src="https://icon.icepanel.io/Technology/svg/Nuxt-JS.svg"/>
                    </a>
                    <a href="https://supabase.com/" target="_blank" class="hover:scale-110 transition-all duration-300">
                        <img alt="techstack icon" class="w-9 h-9 lg:w-16 lg:h-16" src="https://seeklogo.com/images/S/supabase-logo-DCC676FFE2-seeklogo.com.png"/>
                    </a>
                    <a href="https://tailwindcss.com/" target="_blank" class="hover:scale-110 transition-all duration-300">
                        <img alt="techstack icon" class="w-9 h-9 lg:w-16 lg:h-16" src="https://icon.icepanel.io/Technology/svg/Tailwind-CSS.svg"/>
                    </a>
                    <a href="https://vercel.com/" target="_blank" class="hover:scale-110 transition-all duration-300">
                        <img alt="techstack icon" class="w-9 h-9 lg:w-16 lg:h-16" src="https://icon.icepanel.io/Technology/png-shadow-512/Vercel.png"/>
                    </a>
                </div>
            </div>
            <div class="mt-6 xl:mt-9">
                <h2 class="mb-3">Projekt na githubie:</h2>
                <div class="w-fit hover:scale-105 transition-all duration-300">
                    <a href="https://github.com/filipszczech/webdev_portfolio_nuxt" target="_blank" class="">
                        <img alt="techstack icon" class="h-12 lg:h-20" src="https://pngimg.com/uploads/github/github_PNG15.png"/>
                    </a>
                </div>
            </div>
            <div class="mt-6 xl:mt-9">
                <h2 class="mb-3">Paleta kolorów:</h2>
                <div class="flex flex-wrap border-2 border-black w-fit">
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-primary"></div>
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-gray-100"></div>
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-gray-400"></div>
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-gray-700"></div>
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-secondary"></div>
                    <div class="w-9 h-9 lg:w-16 lg:h-16 bg-red-500"></div>
                </div>
            </div>
            <div class="mt-6 xl:mt-9">
                <h2 class="mb-3">Zastosowane paczki:</h2>
                <ul class="list-disc pl-8">
                    <li class="mb-2">@nuxtjs/supabase - implementacja backendu z BaaS od Supabase,</li>
                    <li class="mb-2">@nuxtjs/tailwindcss - style,</li>
                    <li class="mb-2">@nuxt/image - optymalizacja plików zdjęciowych,</li>
                    <li class="mb-2">vee-validate - walidacja formularzy,</li>
                    <li class="mb-2">nuxt-mail - przesyłanie maili,</li>
                    <li class="mb-2">@nuxt/icon - baza darmowych customowalnych ikon,</li>
                    <li class="mb-2">@nuxtjs/i18n - wielojęzykowość,</li>
                    <li class="mb-2">@nuxtjs/color-mode - implementacja trybu ciemnego,</li>
                    <li class="">@nuxt/fonts - baza i łatwe pozyskiwanie fontów.</li>
                </ul>
            </div>
        </section>
    </div>
</template>

<script setup>
    useHead({
        title: "Filip Szczęch | portfolio - o projekcie",
        meta: [
            { name: 'description', content: 'Web developer portfolio - project details.' },
            { property: 'og:description', content: 'Web developer portfolio - project details.' },
        ],
        htmlAttrs: {
            lang: 'pl'
        },
    });
</script>

<style scoped>
    h1, h2 {
        /* font-family: "Russo One", sans-serif; */
        font-family: "Righteous", sans-serif;
    }
</style>
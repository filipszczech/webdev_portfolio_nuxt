<template>
    <div>
        <ModalContainer :isOpen="isProjectModalOpen" @close="closeModal">
            <template #default>
                <h2 class="text-3xl md:text-4xl font-bold mb-3 text-gray-700 dark:text-gray-400 uppercase">{{ currentName }}</h2>
                <p>{{ currentDesc }}</p>
                <img :src="project.img" alt="Project image" class="my-6 w-3/4 mx-auto" />
                <p class="font-semibold">Moje zadania:</p>
                <p class="mb-4">{{ currentTasks }}</p>
                <div class="mb-4">
                    <p class="font-semibold">Stack technologiczny:</p>
